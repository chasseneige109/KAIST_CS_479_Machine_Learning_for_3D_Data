...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        t_bins = self.create_t_bins(num_sample+1, device)  # (num_sample+1,)
        
        noise = torch.rand(num_ray, num_sample, device = device)
        len_bin = t_bins[1] - t_bins[0]
        t_samples = (far - near)*(noise * len_bin + t_bins[:-1]) + near
