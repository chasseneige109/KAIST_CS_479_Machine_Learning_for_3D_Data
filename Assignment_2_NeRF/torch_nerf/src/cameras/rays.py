...
class RayBundle:
    ...
    Ray bundle class.
    """
        # : 는 jaxtyping의 type annotation 문법
        # []는 Python의 Generic 문법인데, batch_size라고 적힌 애들끼리 크기가 같은지 검사하고 통과. 3은 마지막 차원이 3이기만 하면 통과. 아닐시 Run-time error.
...
class RaySamples:
    ...
    def compute_sample_coordinates(self) -> Float[torch.Tensor, "num_ray num_sample 3"]:
        ...
        # nr ns 3 = nr ns 1 x nr 1 3 + nr (1->ns) 3   /// 1 -> ns로 브로드캐스팅
        sample_coords = t_samples.unsqueeze(-1) * ray_directions.unsqueeze(1) + ray_origins.unsqueeze(1)
        # ============================
