...
class GSRasterizer(object):
    ...
    def project_ndc(
        ...
        p_view = points_homo @ w2c # N 4
        p_ndc_non = p_view @ proj_mat  # N 4  
        p_ndc = p_ndc_non / p_ndc_non[:, -1:] # N 4
    ...
    def compute_cov_2d(
        ...
        mean_3d_homo = torch.cat([mean_3d, ones], dim = -1)   
        camera_coord = mean_3d_homo @ w2c
        # ========================================================
        ...
        x = camera_coord[:, 0]
        y = camera_coord[:, 1]
        z = camera_coord[:, 2].clamp(min=1e-6)
        
        J[:, 0, 0] = f_x/z
        J[:, 0, 2] = -f_x*x/(z**2)
        J[:, 1, 1] = f_y/z
        J[:, 1, 2] = -f_y*y/(z**2)
        cov_2d = J @ W @ cov_3d @ W.T @ J.mT
        # ========================================================
    ...
    def render(
        ...
        for h in range(0, camera.image_height, self.tile_size):
            for w in range(0, camera.image_width, self.tile_size):
                ...
                tile_mean_2d = mean_2d[in_mask][sorted_indices]
                tile_cov_2d = cov_2d[in_mask][sorted_indices]
                tile_color = color[in_mask][sorted_indices] # M 3
                tile_opacity = opacities[in_mask][sorted_indices]
                # ========================================================
                
                # ========================================================
                # TODO: Compute the displacement vector from the 2D mean coordinates to the pixel coordinates
                tile_pix = pix_coord[h:h+self.tile_size,w:w+self.tile_size] # tile, tile, 2
                tile_pix = tile_pix.reshape(-1, 2) # tile^2, 2 = P, 2
                
                d_vec = tile_pix[:,None,:] - tile_mean_2d[None, :, :] # (P, M, 2)
                # ========================================================

                # ========================================================
                # TODO: Compute the Gaussian weight for each pixel in the tile
                cov_inv = torch.linalg.inv(tile_cov_2d) # M 2 2
                gaussian_weight = torch.exp(-0.5 * (d_vec[...,None, :] @ cov_inv[None] @ d_vec[...,None]).squeeze(-1).squeeze(-1)) # (P, M)
                # ========================================================
